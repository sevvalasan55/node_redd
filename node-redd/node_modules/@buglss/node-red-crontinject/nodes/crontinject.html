...
<script type="text/html" data-template-name="crontinject">
    <div class="form-row">
    ...
    <div class="form-row cronti-form">
        ...
        <select id="inject-time-type-select">
            ...
            <option value="cronti-on-week" >week of date</option>
            <option value="cronti-on-interval-time" >intervals between dates</option>
            <option value="cronti-on-time" >advanced</option>
            <option value="cronti-on-crontime" >crontime</option>
            <option value="cronti-on-date" >date</option>
    ...
    <div class="form-row inject-time-row hidden cronti-form" id="inject-time-row-cronti-on-date">
        ...
        <p></p>
        <div style="text-align: right;"><span >just for month of date</span><input type="checkbox" id="inject-time-cronti-on-date-isMonthOfDate" style="width: 20px !important;margin-top: 10px;margin-left: 5px;"> </div>
        <p class="clear"></p>
    </div>
    <!-- Cronti End -->
    ...
    <!-- Cronti Begin -->
    <div class="form-row red-ui-editableList-border red-ui-editableList-container" id="crontiMonitor" style="display:none; margin-left: 110px;">
        <div id="loading"></div>
        <!-- Dynamic Content -->
    </div>
    <!-- Cronti End -->
...
<style>
    ...
    .cronti-form select:not(#inject-time-cronti-on-interval-time-units),
    .cronti-form input {
    ...
    select#inject-time-cronti-on-interval-time-units,
    select#inject-time-interval-units {
        float: right;
        width: calc(50% - 60px);
        margin-left: 5px;
    }
    ...
    input#inject-time-cronti-on-interval-time-count,
    input#inject-time-interval-count {
        text-align: right;
        padding-right: 12px;
...
<script type="text/javascript">
    (function() {
        function isReady(method) {
            ...
            switch(method) {
                case "onWeek":
                    bool = $("#inject-time-cronti-on-week-date").val();
                    break;
                case "onIntervalTime":
                    bool = $("#inject-time-cronti-on-interval-time-start-date").val()
                ...
                case "onTime":
                    ...
                    break;
                case "onCrontime":
                    bool = $("#inject-time-cronti-on-crontime").val();
                    break;
                case "onDate":
                    bool = $("#inject-time-cronti-on-date").val();
                    break;
            }
            return bool;
        }

        function getCrontiArgs(method) {
            var args = [];
            switch(method) {
                case "onWeek":
                    args = [
        ...
        function doInject(node, customMsg) {
            ...
            $.ajax({
                ...
                success: function(resp) {
                    RED.notify("Successfully injected: " + label, { type: "success", id: "inject", timeout: 2000 });
                },
                error: function(jqXHR, textStatus, errorThrown) {
                    if(jqXHR.status == 404) {
                        RED.notify(node._("common.notification.error", { message: "node not deployed" }), "error");
                    } else if(jqXHR.status == 500) {
                        RED.notify(node._("common.notification.error", { message: "inject failed, see log for details" }), "error");
                    } else if(jqXHR.status == 0) {
                        RED.notify(node._("common.notification.error", { message: "no response from server" }), "error");
                    } else {
                        RED.notify(node._("common.notification.error", { message: "unexpected error " + (jqXHR.status) + " " + textStatus + " " }), "error");
                    }
                }
            });
        }
        RED.nodes.registerType("crontinject", {
            category: 'common',
            color: "#937DC2",
            defaults: {
                name: { value: "" },
                props: {
                    value: [{ p: "payload" }, { p: "topic", vt: "str" }], validate: function(v, opt) {
                        if(!v || v.length === 0) { return true }
                        for(var i = 0; i < v.length; i++) {
                            if(/msg|flow|global/.test(v[i].vt)) {
                                if(!RED.utils.validatePropertyExpression(v[i].v)) {
                                    return "msg." + v[i].p + ": invalid property expression: " + v[i].v;
                                }
                            } else if(v[i].vt === "json") {
                                try { JSON.parse(v[i].v); }
                                catch(e) {
                                    return "msg." + v[i].p + ": invalid JSON data: " + e.message
                                }
                            } else if(v[i].vt === "num") {
            ...
            label: function(customProps) {
                ...
                if(payloadType === "string" ||
                    ...
                    if((topic !== "") && (topic.length <= 16)) {
                        return topic + ":" + labelLookup[payloadType] + suffix;
                    } else {
                        return labelLookup[payloadType] + suffix;
                    }
            ...
            oneditprepare: function() {
                ...
                var crontiMonitor = $("#crontiMonitor")
                ...
                } else if(node.payloadType === 'string' || node.payloadType === 'none' || node.payloadType === "msg") {
                    payloadType = "str";
                }

                var delayTimer
                function crontiMonitFunc() {
                    ...
                    var args = getCrontiArgs(method)
                    crontiMonitor.show()
                    clearTimeout(delayTimer);
                    if(isReady(method)) {
                        crontiMonitor.html('<div id="loading"></div>')
                        delayTimer = setTimeout(function() {
                            $.ajax({
                                url: "cronjo/next-dates",
                                ...
                                if(xhr.status !== 200) {
                                    crontiMonitor.html("<div><strong>Error: </strong>" + res.error + "</div>")
                                    return
                                }
                                crontiMonitor.html("<h4 class='cronti-header'>Next 5 Dates for '" + method + "'</h4><ul class='cronti-cron--dates'>" + res.nextDates.reduce(function(acc, curr) { var date = new Date(curr); var endDate; if(method === "onIntervalTime") { endDate = new Date(args[1]) }; return acc += (!endDate || date <= endDate) ? ("<li>" + date.toLocaleString() + "</li>") : "" }, "") + "</ul>")
                            }).fail(function(xhr, textStatus, err) {
                                crontiMonitor.html("<div><strong>Error: </strong>" + xhr.responseJSON.error + "</div>")
                            })
                        }, 300);
                    } else {
                        crontiMonitor.html('<div>Parameter Waiting...</div>')
                    }
                }
                ...
                $("#inject-time-type-select").on("change", function() {
                    ...
                    // Cronti
                    var isCronti = id.includes("cronti")
                    if(isCronti) crontiMonitFunc()
                    else crontiMonitor.hide()
                });
                ...
                $("input[id*='cronti'], select[id*='cronti']").on("change spinchange", crontiMonitFunc)
                // Cronti End
                ...
                if(node.inputs) {
                    repeattype = "msg";
                } else if(node.repeat != "" && node.repeat != 0) {
                    repeattype = "interval";
                    var r = "s";
                    var c = node.repeat;
                ...
                var isCronti = repeattype.includes("cronti")

                isCronti ? crontiMonitor.show() : crontiMonitor.hide()

                node.inputs = 0
                node.hasButton = true
                if(repeattype === "msg") {
                    node.inputs = 1
                    node.hasButton = false
                }
                /* */
                ...
                eList.editableList({
                    ...
                    addItem: function(container, i, opt) {
                        ...
                        var propertyValue = $('<input/>', { class: "node-input-prop-property-value", type: "text" })
                            ...
                            .appendTo(row)
                            .typedInput({ default: prop.vt || 'str', types: ['flow', 'global', 'str', 'num', 'bool', 'json', 'bin', 'date', 'env', 'msg'] });
            ...
            oneditsave: function() {
                ...
                node.inputs = type === "msg" ? 1 : 0

                if(type === "none" || type === "msg") {
                    // nothing
